...
export function Nets({scanner,bibleName,AMPscan,CahapterNumbering,scannerLength,AMPscanLength,VWscan,VWscanLength}){
...
let scan = []
 Net.map(content => {
    ...
    for (
      let i = 0;
      i < Math.max(scan.length, scannerLength, AMPscanLength, VWscanLength);
      i++
    ) {
      if (scan[i]) {
